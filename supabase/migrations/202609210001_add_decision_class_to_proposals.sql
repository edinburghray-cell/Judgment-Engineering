...
create index if not exists proposal_versions_decision_class_idx
  on public.proposal_versions (decision_class);
...
begin
  ...
  if nullif(trim(target_decision_class), '') is null then
    raise exception 'Decision class is required';
  end if;

  if trim(target_decision_class) <> 'ai_system_change_approval' then
    raise exception 'Unsupported decision class';
  end if;
...
drop function if exists public.create_proposal(
  text,
  text,
  text,
  text,
