-- Milestone X10-A: allow the trusted JE server path to retrieve
-- contextual Judgments on behalf of a configured JE actor.
--
-- Browser callers continue to use auth.uid().
-- Service-role callers must explicitly provide target_actor_id.
-- The actor identity must therefore be supplied only by the trusted
-- server-side integration layer, never directly from an untrusted browser.
...
begin
  if authenticated_actor is not null then
    current_actor := authenticated_actor;
  elsif current_setting('request.jwt.claim.role', true) = 'service_role'
        and target_actor_id is not null then
    current_actor := target_actor_id;
  else
...
grant execute on function public.retrieve_contextual_judgments(jsonb, integer, uuid)
  to authenticated, service_role;

alter function public.retrieve_contextual_judgments(jsonb, integer, uuid)
  owner to postgres;
